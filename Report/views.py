...
def invoice_pdf_view(request, pk):
    ...
    return HttpResponse(
        ...
        headers={"Content-Disposition": f'attachment; filename="invoice_{pk}.pdf"'},
    )
...
def invoice_excel_view(request, pk):
    ...
    for item in invoice.items.all():
        total = item.quantity * item.price
        ws.append([item.product_name, item.quantity, float(item.price), float(total)])
